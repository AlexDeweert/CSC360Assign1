...
void parseInput( char* input, char*** args, size_t* argc, int* bg_flag ) {
	...
	while( token != NULL ) {
		...
		if( !strcmp(token,"bg") && num_tokens == 0 ) { (*bg_flag)=1; }
...
/* addToList(char*, char*, pid_t )
   Add a child process to a linked list
*/
void addToList( char* command, char* arg1, pid_t pid ) {
	...
	if( NULL != arg1 ) {
		memcpy((*temp).arg1, arg1, strlen(arg1)+1);	
		printf("arg1: %s\n", arg1);
	} 
	else { 
		(*temp).arg1[0] = 0;
		//printf("ARG1 was null!\n");
	}
...
/* printList()
   Print a linked list of child processes
*/
void printList() {
	if( listCount == 1 ) { 
		if( strlen( (*root).arg1) != 0 )
